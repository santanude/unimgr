...
public class OpenFlowUtils {
    ...
    private static final String FLOW_TABLE_NOT_PRESENT_ERROR_MESSAGE =
            "Flow table is not present in node '%s'.";
    private static final String NODE_NOT_AUGMENTED_ERROR_MESSAGE =
            "Node '%s' does not have '%s' augmentation.";
    ...
    public static boolean isTablePreconfigured(Table table) {
        ...
        LOG.debug("Table is " + (isTablePreconfigured ? "" : "not")
                + " preconfigured with default flows.");
    ...
    public static Table getTable(Node node) throws ResourceNotAvailableException {
        ...
        if (flowCapableNode == null) {
            LOG.warn(String.format(NODE_NOT_AUGMENTED_ERROR_MESSAGE, nodeId,
                    FlowCapableNode.class.toString()));
            throw new ResourceNotAvailableException(String.format(NODE_NOT_AUGMENTED_ERROR_MESSAGE,
                    nodeId, FlowCapableNode.class.toString()));
        }

        Optional<Table> flowTable = flowCapableNode.getTable().stream()
                .filter(table -> table.getId().equals(FLOW_TABLE_ID)).findFirst();
        if (!flowTable.isPresent()) {
            LOG.warn(String.format(FLOW_TABLE_NOT_PRESENT_ERROR_MESSAGE, nodeId));
            throw new ResourceNotAvailableException(
                    String.format(FLOW_TABLE_NOT_PRESENT_ERROR_MESSAGE, nodeId));
        }
     ...
     */
    public static List<Flow> getVlanFlows(String servicePort, int internalVlanId,
        ...
        flows.add(createVlanIngressFlow(servicePort, internalVlanId, externalVlanId, serviceName,
    ...
    /**
     * Returns list of flows - full-mesh - created on the base of list of provided links and default
     * dropping all flow
     *
    ...
    /**
     * Returns list of flows installed in flow table. Flows which match LLDP packets are filtered
     * out.
     *
     * @param table flow table
     * @return list of flows
    ...
    public static List<Flow> getExistingFlowsWithoutLLDP(Table table) {
        return table.getFlow().stream().map(x -> new FlowBuilder(x).build()).filter(flow -> {
            try {
                if (flow.getMatch().getEthernetMatch().getEthernetType().getType().getValue()
                        .equals(Long.valueOf(EtherTypes.LLDP.intValue())))
                    return false;
    ...
    private static List<Flow> createInterswitchFlows(List<Link> interswitchLinks) {
        ...
        List<String> portIds = interswitchLinks.stream().map(link -> link.getLinkId().getValue())
                .collect(Collectors.toList());

        for (String portId : portIds) {
            List<Action> outputActions =
                    portIds.stream().filter(outputPortId -> !outputPortId.equalsIgnoreCase(portId))
                            .map(outputPortId -> ActionUtils.createOutputAction(outputPortId,
                                    portIds.indexOf(outputPortId)))
                            .collect(Collectors.toList());
            FlowId flowId = new FlowId(INTERSWITCH_FLOW_ID_PREFIX + "-" + portId);
            Flow flow = new FlowBuilder().setId(flowId).setKey(new FlowKey(flowId))
                    .setTableId(FLOW_TABLE_ID).setPriority(INTERSWITCH_FLOW_PRIORITY)
                    .setMatch(MatchUtils.createInPortMatch(portId))
                    .setInstructions(ActionUtils.createInstructions(outputActions)).build();
            flows.add(flow);
        }
    ...
    private static Flow createDefaultFlow() {
        ...
        return new FlowBuilder().setId(dropFlowId).setKey(new FlowKey(dropFlowId))
                .setTableId(FLOW_TABLE_ID).setPriority(DROP_FLOW_PRIORITY)
    ...
    private static List<Flow> createVlanPassingFlows(String outputPort, int internalVlanId,
            Optional<Integer> externalVlanId, String serviceName, List<Link> interswitchLinks) {
        return interswitchLinks.stream().map(link -> createVlanPassingFlow(outputPort,
                link.getLinkId().getValue(), internalVlanId, externalVlanId, serviceName))
                .collect(Collectors.toList());
    }
    ...
    private static Flow createVlanPassingFlow(String outputPort, String inputPort,
            int internalVlanId, Optional<Integer> externalVlanId, String serviceName) {
        // Create list of actions and VLAN match
        LOG.info(" inside createVlanPassingFlow () outputPort{}, inputPort{} ", outputPort,
                inputPort);
    ...
    public static Flow createVlanIngressFlow(String inputPort, int internalVlanId,
        ...
        List<String> outputPortIds =
                interswitchLinks.stream().map(link -> link.getLinkId().getValue())
        ...
        List<Action> actions = new ArrayList<>();
        int actionOrder = 0;
        // 1. Create VLAN actions performing VLAN translation when service VLAN is already used in
        // OvSwitch network
        if (externalVlanId.isPresent()) {
            actions.add(ActionUtils.createPopVlanAction(actionOrder++));
        }
        ...
        return new FlowBuilder().setId(flowId).setKey(new FlowKey(flowId)).setTableId(FLOW_TABLE_ID)
                ...
                .setMatch(externalVlanId.isPresent()
                        ? MatchUtils.createVlanMatch(externalVlanId.get(), inputPort)
                        : MatchUtils.createInPortMatch(inputPort))
                .setInstructions(ActionUtils.createInstructions(actions)).build();
    }
