...
public class AbstractL2vpnBridgeDomainActivatorTest {
	...
	absl2vpnBridgeDomain = Mockito.mock(AbstractL2vpnBridgeDomainActivator.class,Mockito.CALLS_REAL_METHODS);
	endPoints=new ArrayList<EndPoint>();
	isExclusive=true;
	serviceType=ServiceType.POINTTOPOINTCONNECTIVITY.getName();
	...
	public void activateTest() throws TransactionCommitFailedException, ResourceActivatorException {
        ...
        PowerMockito.doNothing().when(transaction).merge(any(LogicalDatastoreType.class), any(InstanceIdentifier.class),any(InterfaceConfigurations.class));
        PowerMockito.doNothing().when(transaction).merge(any(LogicalDatastoreType.class), any(InstanceIdentifier.class),Mockito.anyObject());
...
@Test
public void deactivateTest() throws TransactionCommitFailedException, ResourceActivatorException{
    ...
    PowerMockito.when(optBroker.get()).thenReturn(databroker);
    PowerMockito.when(databroker.newWriteOnlyTransaction()).thenReturn(transaction);
    PowerMockito.doNothing().when(transaction).delete(any(LogicalDatastoreType.class), any(InstanceIdentifier.class));
    PowerMockito.doNothing().when(transaction).delete(any(LogicalDatastoreType.class), any(InstanceIdentifier.class));
    CheckedFuture checkedFuture=Mockito.mock(CheckedFuture.class);
    PowerMockito.when(transaction.submit()).thenReturn(checkedFuture);
