...
import org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.*;
import org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.get.link.details.output.LinkBuilder;
import org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.get.node.details.output.NodeBuilder;
import org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.get.topology.details.output.TopologyBuilder;
import org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.topology.Link;
import org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.topology.LinkKey;
...
public class TapiTopologyServiceImpl implements TapiTopologyService, AutoCloseable {
            ...
            if(ctx.isPresent()) {
                ...
                out = RpcResultBuilder.success(
                        new GetTopologyListOutputBuilder()
                                .setTopology(topologies.stream().map(t -> new org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.get.topology.list.output.TopologyBuilder(t).build()).collect(Collectors.toList()))
                ).build();
            }
        } catch (ReadFailedException e) {
    ...
    public Future<RpcResult<GetTopologyDetailsOutput>> getTopologyDetails(GetTopologyDetailsInput input) {
        return executor.submit(() -> {
            NrpDao nrpDao = new NrpDao(broker.newReadOnlyTransaction());
            org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.topology.context.Topology topo = nrpDao.getTopology(input.getTopologyIdOrName());
