...
import org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.get.link.details.output.Link;
...
public class TapiTopologyServiceImplIntTest extends AbstractTestWithTopo {
    ...
    public void getLink() throws TransactionCommitFailedException, ExecutionException, InterruptedException {
        ...
        org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev180216.topology.Link l = l(tx, uuid1 + ":1", uuid2 + ":1", OperationalState.ENABLED);
        tx.submit().checkedGet();
        ...
        Set<String> nepUuids = link.getNodeEdgePoint().stream().map(u -> u.getValue()).collect(Collectors.toSet());
        Assert.assertTrue(nepUuids.contains(uuid1 + ":1"));
        Assert.assertTrue(nepUuids.contains(uuid2 + ":1"));
    }
